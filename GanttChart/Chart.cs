...
namespace Braincase.GanttChart
{
    ...
    public partial class Chart : UserControl
    {
        ...
        public Chart()
        {
            ...
            _mViewport = new ControlViewport(this) { WheelDelta = 20 };
            ...
            TaskFormat = new GanttChart.TaskFormat() {
                Color = Brushes.Black,
                Border = Pens.Black,
                BackFill = Brushes.CornflowerBlue,
                ForeFill = Brushes.DarkSeaGreen,
                SlackFill = new System.Drawing.Drawing2D.HatchBrush(System.Drawing.Drawing2D.HatchStyle.LightDownwardDiagonal, Color.DarkSeaGreen, Color.Transparent)
            };
        ...
        /// <summary>
        /// GridView와 Chart 가 같이 움직때 사용
        /// </summary>
        /// <param name="newValue"></param>
        public void MoveScrollY(int newValue)
        {
            _mViewport.Y = newValue * 23;
        }

        public float MoveScrolY2()
        {
            return _mViewport.Y;
        }
        /// <summary>
        ...
        protected virtual void OnTaskMouseOver(TaskMouseEventArgs e)
        {
            ...
            if (_mTaskToolTip.ContainsKey(task))
            {
                ...
                //_mOverlay.ShowToolTip(_mViewport.DeviceToWorldCoord(e.Location), "가나다");
                this.Invalidate();
            }
        }
        /// <summary>
        ...
        private void ___GetLabelFormat(DateTime datetime, DateTime datetimeprev, out LabelFormat minor, out LabelFormat major)
        {
            ...
            switch (TimeResolution)
            {
                ...
                default: // case TimeResolution.Day: -- to implement other TimeResolutions, add to this function or listen to the the PaintTimeline event
                    minor.Text = datetime.ToString("yyyy MM dd").Substring(8, 2).ToUpper(); //ShortDays[datetime.DayOfWeek];
                    if (datetime.DayOfWeek == DayOfWeek.Sunday) major.Text = datetime.ToString("yyyy-MM");
                    break;
            }
        }
